...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        help="Which model will be trained (classifier/synthesizer)")
    parser.add_argument(
        "--epochs",
    ...
    if args.model in ['classification', 'classifier']:
        # Device
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        # Load model
        best_model = last_checkpoint('checkpoint/')
        model = GenocidalClassifier('models/classifier/lastmodel.pt')
        model.to(device)
